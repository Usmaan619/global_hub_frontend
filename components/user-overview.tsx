...
export function UserOverview() {
  const { currentUser, getUsersByAdmin, dataEntries, getDataEntriesByUser } = useAuthStore()
  ...
  const selectedUserEntries = selectedUser ? getDataEntriesByUser(selectedUser.id) : []
      ...
      <Dialog open={viewUserEntries} onOpenChange={setViewUserEntries}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>All data entries created by {selectedUser?.name}</DialogDescription>
          </DialogHeader>
          <div className="max-h-96 overflow-y-auto">
            {selectedUserEntries.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">This user hasn't created any entries yet.</p>
            ) : (
